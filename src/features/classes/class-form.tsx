...
export function ClassForm({ session }: { session?: ClassSession }) {
  ...
  return (
    <form action={submit} className="space-y-6">
      ...
      <p className="text-sm text-neutral-600">시작·종료 일시는 한국 시간 기준입니다. 메모를 제외한 항목은 필수입니다.</p>
      <fieldset disabled={pending} className="grid min-w-0 gap-5 sm:grid-cols-2">
        {fields.map(({ name, label, ...props }) => (
          <div key={name} className="min-w-0">
            <label htmlFor={name} className="text-sm font-semibold text-neutral-700">{label}</label>
            <input {...props} id={name} name={name} required defaultValue={values[name]}
              ...
              className="mt-2 block w-full min-w-0 rounded-lg border border-neutral-300 bg-white px-3 py-3 outline-none focus:border-black focus:ring-2 focus:ring-neutral-200" />
            {state.fieldErrors?.[name] && <p id={name + "-error"} className="mt-1 text-sm text-red-700">{state.fieldErrors[name]}</p>}
          </div>
        ))}
        <div className="sm:col-span-2">
          <label htmlFor="memo" className="text-sm font-semibold text-neutral-700">메모 (선택)</label>
          <textarea id="memo" name="memo" rows={4} defaultValue={values.memo}
            className="mt-2 w-full rounded-lg border border-neutral-300 px-3 py-3 outline-none focus:border-black focus:ring-2 focus:ring-neutral-200" />
        </div>
      </fieldset>
      {state.error && <p role="alert" className="rounded-lg bg-red-50 p-4 text-sm text-red-700">{state.error}</p>}
      <div className="flex flex-wrap gap-3">
        <button disabled={pending} className="rounded-lg bg-black px-6 py-3 font-semibold text-white disabled:opacity-60">
          {pending ? "저장 중…" : session ? "수정 저장" : "수업 등록"}
        </button>
        <Link href={session ? "/classes/" + session.id : "/classes"} className="rounded-lg border border-neutral-300 px-6 py-3 text-neutral-700">돌아가기</Link>
      </div>
    </form>
  );
}
